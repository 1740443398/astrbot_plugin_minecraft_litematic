...
class Render3DManager:
    ...
    def render_multi_view(self, file_path: str,
                          window_size: Optional[Tuple[int, int]] = None,
                          native_textures: bool = False,
                          native_max_size: Optional[Tuple[int, int]] = None) -> list:
        try:
            schematic = Schematic.load(file_path)
            model_builder = ModelBuilder()
            if not model_builder.build_from_litematic(schematic):
            ...
            color_mapper = ColorMapper(self.resource_dir)
            renderer = PyVistaRenderer(
                model_data, surface_data, color_mapper,
                resource_dir=self.resource_dir, native_textures=native_textures
            )
            ...
            iso_camera = (center_x + distance, center_y + distance, center_z + distance)
            ...
            results = []
            first_view = True
            for view_name, camera_pos, up_vec in views:
                try:
                    if first_view:
                        image = renderer.render_scene(
                            camera_position=[camera_pos, (center_x, center_y, center_z), up_vec],
                            window_size=window_size
                        )
                        first_view = False
                    else:
                    ...
                    if image is None:
                        logger.warning(f"渲染 {view_name} 视角返回None")
                        continue

                    with tempfile.NamedTemporaryFile(suffix='.png', delete=False) as tmp:
                        img_path = tmp.name
                    image.save(img_path)
                    results.append((view_name, img_path))
                except Exception as e:
            ...
            logger.error(f"多视角渲染时出现未知错误: {e}")
            raise RenderError(f"多视角渲染失败: {str(e)}", code=2000)

    async def render_multi_view_async(self, file_path: str,
                                      window_size: Optional[Tuple[int, int]] = None,
                                      native_textures: bool = False,
                                      native_max_size: Optional[Tuple[int, int]] = None) -> list:
        return await asyncio.to_thread(
            self.render_multi_view,
            file_path, window_size, native_textures, native_max_size
        )
